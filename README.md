...
&emsp;&emsp;首先，进行数据开发，构建LRFMP用户分层模型，得到所有用户的L（生命周期）、R（消费活跃度）、F（消费频率）、M（平均消费金额）、P（平均消费利润）值；然后，汇总得到LRFMP值；最后，LRFMC模型构建之后使用了经典的聚类算法K-Means算法来对客户进行细分，而不是传统的来与参考值对比进行手工分类，提高准确率和效率，从而实现用户价值等级分层，进行精准的价格和服务设置。
